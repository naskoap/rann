...
-   A basic-graph-based visualization tool is great for small scale work, but not as good for production level work in industry, where an interactive dashboard is preferred.
...
Optuna has an interactive dashboard that opens up in a web browser. This is not easily captured with images, so instead, we created the dashboard and then recording ourselves scrolling through it and interacting with different graphs. The recording can be viewed [here](https://drive.google.com/file/d/1DsS_wibUr-DK0hWDWvgwG1eJEUXoL4r5/view?usp=share_link).
...
-   The history plot is much easier to follow than Hyperopt, as it has an clear trend for the most optimal value over time, which is separated out from the rest of the data points.
