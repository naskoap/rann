...
In this project, we compare three different options for the step of hyperparameter tuning within a Python-based machine learning pipeline: 1) Grid Search (which is a standard tool built into machine learning modules in Python, and which represents our control condition), and two external libraries, 2) Hyperopt, and 3) Optuna, which users might apply to a project if they want more control over their tuning process or want to try out a more advanced or efficient algorithm for hyperparamter tuning.

Both Hyperopt and Optuna use advanced algorithms for tuning that involve Bayesian optimization, and both claim to be faster and better performing than standard tools like grid search. Both also include UI capabilities that help the user to visualize the tuning process, something which Grid Search does not provide. For more information on how Hyperopt and Optuna work, and how they differ from Grid Search, see the README file located [here](https://github.com/naskoap/rann/tree/main/experiment).

For our comparison, we start with a code review of both external tools based on the code available in their public repositories, reviewing both the repository itself (in terms of maintenance and user interaction) and non-functional requirements. We then conduct an experimental comparison where we train and tune machine learning models on both regression and classification datasets using Scikit-Learn. We compare their performance both to each other and to a standard Grid Search in terms of processing time and model performance. As part of this experiment, we also generate the UI components provided by Hyperopt and Optuna and review their pros and cons.
...
There are several pull requests that have been sitting in the repository for a long time, including one as old as August 2020. Similarly, there are 375 open issues with a few dozen being **more than a decade old**!! This indicates a somewhat [poor]{.underline} level of maintenance.
...
A [Snyk analysis](https://snyk.io/advisor/python/hyperopt#security) found 0 direct vulnerabilities in the code and 3 indirect vulnerabilities, all associated with software dependencies. These indirect vulnerabilities were classified as **Low** severity. Overall, this indicates that Hyperopt is quite secure.
...
There also appear to be some tests that get *skipped*. `test_plotting.py` has a *try* statement in the imports, and [skips]{.underline} the test if there is an issue with `matplotlib`. Having a built in feature to skip a test is not good practice. If the authors know why this `matplotlib` error happens, which thy appear to, it should be possible to write the test in such a way as to get around it.
...
A [Snyk analysis](https://snyk.io/advisor/python/optuna#security) found 0 direct vulnerabilities in the code and 3 indirect vulnerabilities, all associated with software dependencies. These indirect vulnerabilities were classified as **Low** severity. Overall, this indicates that even though it has many dependencies, Optuna is quite secure, just like Hyperopt.
...
Examples like this were one area Hyperopt fell quite short. The official documentation had only one example, and a link to additional examples in the README directed to a deprecated page. Optuna, on the other hand, has an [**entire repository**](https://github.com/optuna/optuna-examples) dedicated to examples, in addition to the example on the README. This repository is linked in the README.
...
There is also good documentation provided for how to generate the UI dashboard step by step,
...
This wide level of code coverage is actually verifiable by the user, since Optuna uses a `codecov` plugin on the repository. `codecov` is a tool that checks test coverage *every time* a new commit is added to the repository. Optuna's `codecov` page is linked [here](https://app.codecov.io/gh/optuna/optuna).
...
#### Extensibility/Scalability **`(Score: 7/10)`**
...
Optuna also has a built in UI dashboard. The repository storing its code is linked [here](https://github.com/optuna/optuna-dashboard). This dashboard is super simple to use. The user need only define a location to store the results of the study/parameter tuning process, run `pip install optuna-dashboard` and then run a single line of code to get the dashboard running. It appears like you can set which IP address the dashboard gets hosted on, allowing for customization.
...
However, one area where Optuna falls short of Hyperopt is with integrations to other common ML tools. Hyperopt has explicit examples and information provided on how to integrate with tools like MLFlow that are commonly used in production environments. Optuna has no such information provided, and it is not clear to users how to conduct these integrations.

In terms of extensibility, Optuna does very well. Just like Hyperopt, all of the code is publicly available on GitHub--both for the original tool itself and for additional features like the dashboard--making it easy to contribute.
...
For the [classification]{.underline} problem, we used data on [hospital readmissions](https://www.kaggle.com/code/iabhishekofficial/prediction-on-hospital-readmission), predicting whether someone will be readmitted to the hospital based on data from their last stay. To do this, we used the `RandomForestClassifier` model from `scikit-learn`. A snapshot of some of this data can be seen below:

![](images/hospital-data.png){fig-align="center"}
...
| Tuning Time | 9 min, 24 sec | 3 min, 35 sec | 3 min, 25 sec |

: Classification Experiment Metrics By Tuning Tool
...
For the [regression]{.underline} problem, we used data on [CO2 emissions](https://www.kaggle.com/datasets/debajyotipodder/co2-emission-by-vehicles) from vehicles, predicting the CO2 output from a vehicle based on its engine size, fuel type, vehicle type, etc. To do this, we used the `RandomForestRegressor` model from `scikit-learn.` A snapshot of this data can be seen below:

![](images/car-data.png){fig-align="center"}
...
: Regression Experiment Metrics By Tuning Tool
...
During the experiment, we implemented the available UI features in each of our two external tools. In this section, we summarize what those features are and the pros and cons we found for each based on our use. Finally, we rate each tool from 1-10 in terms of four different categories:

1.  **Ease of Coding** -- How easy it was to generate the plots coming from no prior experience with the tool.
2.  **Appearance** -- How nice the visuals look / whether any graphs are clearly labeled / how easy it is to understand the graphs just by looking at them.
3.  **Added Information** -- How the tools assist the user by giving useful information about the tuning process/
4.  **Productionalizability** -- How "ready" the UI output is to be used in a production type environment.

## Hyperopt

Hyperopt has three plotting functions, the results of which are displayed below:

![](images/hyperopt-ui.png){fig-align="center"}

### Pros

-   Graphs are really easy to generate (requiring just a line of code each).

-   For those working in tools like Jupyter notebooks (which are common in ML), the graphs can be automatically embedded in the notebook and easily shared.

### Cons

-   Visualizations are just built on top of basic `matplotlib`, which users could graph by themselves without much effort. No sort of advanced visualization capabilities are available.

-   The scatterplot of loss history is a bit hard to read, as the points blend together and it is hard to see which point came before a previous one.

-   For the third graph (right), there is no documentation in the tool telling the user what this is supposed to show, so it is unclear what this one even does.

-   Some of the information does not seem all that useful. Not sure why someone would need to know the information provided by the loss histogram, as this would not really help influence how a model gets tuned (since we mostly care about just the optimal value, rather than the distribution).

-   A basic-graph-based visualization tool is great for small scale work, but not as good for production level work in industry.

### Scores

-   **Ease of Coding** -- `6/10` (Only takes a few lines of code, but the documentation is not clear on what steps are required)

-   **Appearance** -- `3/10` (Some graphs are missing titles, all are super basic and no nicer than what the user could generate themselves, and some of the graphs are a little clunky and hard to read)

-   **Added Information** -- `2/10` (The third graph is completely unintelligible and the others don't show that much useful information)

-   **Productionalizability** -- `2/10` (The graphs only work well for embedding in a notebook and don't look anywhere near production ready)

## Optuna

Optuna has a web-based interactive dashboard. To capture the full experience, we created the dashboard and then recording ourselves scrolling through it and interacting with different graphs. The recording can be viewed [here](https://drive.google.com/file/d/1DsS_wibUr-DK0hWDWvgwG1eJEUXoL4r5/view?usp=share_link).

![](images/optuna-graph.png){fig-align="center" width="481"}

### Pros

-   Wide variety of plots showing lots of different information (and much more information than provided by Hyperopt). Some plots seem very useful, such as one that shows how important each different parameter was to the overall tuning process.

-   The history plot (shown in the image above) is much easier to follow than the equivalent plot in Hyperopt, as it has an clear trend for the most optimal value over time, which is separated out from the rest of the data points.

-   Plots are interactive, allowing users to hover around data points to access further information on these points and visualize them in scale.

-   The dashboard format is easier to productionalize than basic graphs.

### Cons

-   There is not much documentation on what each of the graphs shows, so even though there is a lot of information, users may not know how to use it without lots of domain knowledge.

-   The large number of plots might feel overwhelming, especially if users only care about 2 or 3 of them.

-   There is no easy way to embed these plots in a notebook for smaller projects.

### Scores

-   **Ease of Coding** -- `9/10` (Great documentation, only takes a few lines of code)

-   **Appearance** -- `7/10` (Graphs are titled and have clear axes, but scrolling-based system to move between graphs is not necessarily optimal and no way to hide graphs users don't care about)

-   **Added Information** -- `7/10` (Lots of useful graphs, but this information is not always usable without prior domain knowledge & documentation is lacking)

-   **Productionalizability** -- `7/10` (Appearance and web based format are perfect for production environments, but no documentation on how to deploy this other than locally)

# Discussion

Based on our experimental testing, both Hyperopt and Optuna appear to show significantly improved processing speed compared to grid search, as well as advanced features, such as UI components and the ability to custom-define a metric to maximize or minimize.

Optuna claims to have even faster run times due to its parallel processing, but we did not note a significant difference between the speed of Optuna and Hyperopt in this experiment. However, this is potentially because we were working with small data (\<10,000 rows), where parallel processing is less relevant than with large data (\>1 million rows).

For our classification problem, Optuna and Hyperopt performed the approximately the same in terms of model score as Grid Search, but both tools performed better than Grid Search for the regression problem when considering the optimized metric (MSE).

Similar to tuning speed, we did not identify a clear winner between Optuna and Hyperopt in terms of model score. For some metrics, Optuna did slightly better, and for others Hyperopt did, though differences were minimal.

Although performance was relatively even, Optuna was the clear winner of both the code review and UI review, specifically for production-type environments. Optuna has *much* better documentation, a much more advanced and informative UI interface, and is regularly maintained. Hyperopt's documentation is lacking, the code has not been updated in years, and the UI options are very basic and sometimes confusing.

If we were to make a recommendation based on this knowledge, we would say the following:

-   Don't use Grid Search unless working with *very* small data or in situations where you want to fully stay within the `scikit-learn` library. It is *extremely* slow compared to external tools, and does not offer improved performance.

-   If doing a small-scale project in a Jupyter Notebook (or similar), Hyperopt would be a good choice. It is easy to embed the results/plots directly in the notebook, which is great for sharing. Kaggle competitions, class assignments, personal projects, etc. are all good candidates. If working in a production environment that uses MLFlow, it might also be a good option due to easy integration, though production work often prioritizes using software that is regularly maintained.

-   If doing a production project anywhere *other* than MLFlow, *or* one involving large data, Optuna is probably the better choice. It may offer speed advantages in processing large models and the UI dashboard is much more production-ready. The code and results are also easier to explain to stakeholders than Hyperopt.

-   If using Hyperopt, seek out external resources for information on how to code the optimization and be *very* careful about specifying a metric, as it is easy to accidentally set the optimization in the wrong direction.

-   For Optuna, the official examples should provide a solid starting point for documentation, but there are also other resources online if needed. These will specifically be necessary if trying to integrate Optuna with other tools, as this information is not provided in the documentation.

# References

### Hyperopt Official Documentation

-   Official Repository: https://github.com/hyperopt/hyperopt

-   Official Documentation: http://hyperopt.github.io/hyperopt/

-   Hyperopt: a Python library for model selection and hyperparameter optimization (Bergstra et al, 2015): https://dx.doi.org/10.1088/1749-4699/8/1/014008

### Optuna Official Documentation

-   Official Repository: https://github.com/optuna/optuna

-   Code Examples: https://github.com/optuna/optuna-examples/

-   UI Dashboard: https://github.com/optuna/optuna-dashboard

-   Website: https://optuna.org

-   Optuna: A Next-generation Hyperparameter Optimization Framework (Akiba et al, 2019): https://dl.acm.org/doi/pdf/10.1145/3292500.3330701

### Other Resources + Unofficial Coding Examples

-   A Comparative Study of Hyper-Parameter Optimization Tools (Shekhar et al, 2022): https://arxiv.org/pdf/2201.06433.pdf

-   Scikit-Learn Grid Search Documentation: https://scikit-learn.org/stable/modules/grid_search.html#exhaustive-grid-search

-   Grid Search Examples: https://towardsdatascience.com/hyperparameter-tuning-the-random-forest-in-python-using-scikit-learn-28d2aa77dd74

-   Unofficial Code Examples for Optuna and Hyperopt:
